...
class MPCC {
 ...
 protected:
  ...
  static double constraint(const std::vector<double>& x,
                           std::vector<double>& grad, void* data);

  static void multi_constraint(unsigned m, double* result, unsigned n,
                               const double* x, double* grad, void* f_data);

  // objective
  static autodiff::real eval_objective(const autodiff::ArrayXreal& x,
                                       void* data);

  // constraints
  // static double eval_h_func(const std::vector<double> &x, void *data);
  // static std::vector<double> eval_cbf_constraint(const std::vector<double>
  // &vars, void *data);
  static autodiff::VectorXreal eval_cbf_constraint(
      const autodiff::VectorXreal& vars, void* data);
  static autodiff::VectorXreal eval_constraint(const autodiff::VectorXreal& x,
                                               void* data);
