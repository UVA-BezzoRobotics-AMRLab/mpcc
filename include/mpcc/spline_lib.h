...
class BSpline {
  ...
  BSpline(const std::vector<double>& x, const std::vector<double>& y) {
    if (x.size() != y.size() || x.size() < 4) {
      throw std::invalid_argument(
          "Input vectors must have the same size and at least 4 points.");
    }
 ...
 private:
  ...
  double evaluateBasis(int i, int k, double t,
                       const std::vector<double>& knotVec) const {
    if (k == 1) {
  ...
  std::vector<double> computeControlPoints(const std::vector<double>& x,
