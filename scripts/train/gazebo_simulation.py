...
class GazeboSimulation():
    ...
    def __init__(self, init_position=[0, 0, 0], collision_topic='/collision'):
        ...
        self._model_state = rospy.ServiceProxy(
            '/gazebo/get_model_state', GetModelState)

        self._init_model_state = create_model_state(
            init_position[0], init_position[1], 0, init_position[2])
        ...
        while data is None:
            try:
                data = rospy.wait_for_message(
                    'front/scan', LaserScan, timeout=5)
            except:
                pass
        return data
