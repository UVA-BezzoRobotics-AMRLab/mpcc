...
from LP_codegen import cpg_module
...
def cpg_solve(prob, updated_params=None, **kwargs):
    ...
    if updated_params is None:
        updated_params = ["Domain", "A_mat", "b_vec"]
    ...
    par.Domain = list(param_dict["Domain"].value.flatten(order="F"))
    par.A_mat = list(param_dict["A_mat"].value.flatten(order="F"))
    par.b_vec = list(param_dict["b_vec"].value.flatten(order="F"))
    ...
    prob.var_dict['var2'].save_value(np.array(res.cpg_prim.var2).reshape(7))
    prob.constraints[0].save_dual_value(np.array(res.cpg_dual.d0).reshape(200))
