...
#include "uav_mpc/unicycle_mpcc/acados_solver_unicycle_model.h"
...
void interpolate_and_compute_derivatives(const Eigen::MatrixXd& reference)
{
    ...
    Eigen::ArrayXd time_steps = Eigen::ArrayXd::LinSpaced(n, 0, 1); // Normalize time steps to [0, 1]

    // Create splines for x_ref and y_ref
    Eigen::Spline<double, 1> spline_x = Eigen::SplineFitting<Eigen::Spline<double, 1>>::Interpolate(reference.col(0).transpose(), 3, time_steps);
    Eigen::Spline<double, 1> spline_y = Eigen::SplineFitting<Eigen::Spline<double, 1>>::Interpolate(reference.col(1).transpose(), 3, time_steps);
...
int main()
{
    ...
    int status = unicycle_model_acados_create_with_discretization(acados_ocp_capsule, N, new_time_steps);
