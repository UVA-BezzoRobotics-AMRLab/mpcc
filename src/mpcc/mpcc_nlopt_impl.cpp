...
{
    _segments = segments;
    if (segments.size() != 0) _ds = abs(segments[0].s1 - segments[0].s0);
}
...
autodiff::real MPCC::eval_objective(const autodiff::ArrayXreal &x, void *data)
{
    ...
    for (int i = 0; i < obj._mpc_steps; ++i)
    {
        ...
        autodiff::real ref_x =
            obj._reference_tk[0].spline(autodiff::val(x[obj._s_start + obj._ind_inc * i]));
        autodiff::real ref_y =
            obj._reference_tk[1].spline(autodiff::val(x[obj._s_start + obj._ind_inc * i]));
        ...
        autodiff::real e_c     = sin(ref_phi) * (x[obj._x_start + obj._ind_inc * i] - ref_x) -
                             cos(ref_phi) * (x[obj._y_start + obj._ind_inc * i] - ref_y);
        autodiff::real e_l = -cos(ref_phi) * (x[obj._x_start + obj._ind_inc * i] - ref_x) -
...
autodiff::VectorXreal MPCC::eval_constraint(const autodiff::VectorXreal &x, void *data)
{
    ...
    for (int i = 0; i < obj._mpc_steps - 1; ++i)
    {
        ...
        result[obj._theta_start + (obj._ind_inc - 3) * (i + 1)] =
            atan2(sin(theta1 - (theta0 + w0 * obj._dt)), cos(theta1 - (theta0 + w0 * obj._dt)));
...
void MPCC::multi_constraint(unsigned m, double *result, unsigned n, const double *x,
                            double *grad, void *f_data)
{
    ...
    {
        // std::cout << "\twith gradient" << std::endl;
        Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> > Jx(
            grad, m, n);
...
std::vector<double> MPCC::solve(const Eigen::VectorXd &state)
{
    ...
    _state << x0[_x_start], x0[_y_start], x0[_theta_start], x0[_v_start], x0[_s_start],
        x0[_s_dot_start];
