...
class FG_eval {
 public:
  ...
  void load_params(const std::map<std::string, double>& params) {
    ...
    _w_etheta = params.find("W_ETHETA") != params.end() ? params.at("W_ETHETA")
                                                        : _w_etheta;
    ...
    _use_cbf = params.find("USE_CBF") != params.end() ? params.at("USE_CBF")
                                                      : _use_cbf;
  ...
  void set_tubes(const std::vector<SplineWrapper>& tubes) { _tubes = tubes; }
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < _mpc_steps; i++) {
      ...
      if (_use_eigen) {
        ...
        ref_vx = _reference[0]
                     .derivatives(CppAD::Value(CppAD::Var2Par(s)), 1)
                     .coeff(1);
        ref_vy = _reference[1]
                     .derivatives(CppAD::Value(CppAD::Var2Par(s)), 1)
                     .coeff(1);
        ...
        ref_vx =
            _reference_tk[0].spline.deriv(1, CppAD::Value(CppAD::Var2Par(s)));
        ref_vy =
            _reference_tk[1].spline.deriv(1, CppAD::Value(CppAD::Var2Par(s)));
        ref_phi = CppAD::atan2(ref_vy, ref_vx);
      }

      AD<double> e_c =
          CppAD::sin(ref_phi) * (x - ref_x) - CppAD::cos(ref_phi) * (y - ref_y);
      AD<double> e_l = (-CppAD::cos(ref_phi) * (x - ref_x) -
                        CppAD::sin(ref_phi) * (y - ref_y));
    ...
    for (int i = 0; i < _mpc_steps - 2; i++) {
      ...
      fg[0] +=
          _w_linvel_d *
          CppAD::pow(vars[_linacc_start + i + 1] - vars[_linacc_start + i], 2);
      fg[0] +=
          _w_linvel_d *
          CppAD::pow(vars[_s_ddot_start + i + 1] - vars[_s_ddot_start + i], 2);
    }
    ...
    for (int i = 0; i < _mpc_steps - 1; i++) {
      ...
      if (_use_cbf) {
        ...
        AD<double> dist = _dist_grid_ptr->atPositionSafe(
                              CppAD::Value(CppAD::Var2Par(x0)),
                              CppAD::Value(CppAD::Var2Par(y0)), true) +
                          1e-6;
        AD<double> D = dist - padding - 1e-6;

        distmap::DistanceMap::Gradient grad =
            _dist_grid_ptr->gradientAtPosition(CppAD::Value(CppAD::Var2Par(x0)),
                                               CppAD::Value(CppAD::Var2Par(y0)),
                                               true);
        ...
        AD<double> p0 =
            (dx * CppAD::cos(theta0) + dy * CppAD::sin(theta0)) / dist;
        AD<double> pp =
            (-dy * CppAD::cos(theta0) + dx * CppAD::sin(theta0)) / dist;
        AD<double> P = p0 + v0 * d2;

        AD<double> Lfh1 = (CppAD::exp(-P) * xdot / dist) *
                          (-dx + D * (CppAD::cos(theta0) - dx * p0 / dist));
        AD<double> Lfh2 = (CppAD::exp(-P) * ydot / dist) *
                          (-dy + D * (CppAD::sin(theta0) - dy * p0 / dist));
        ...
        fg[2 + _s_dot_start + _mpc_steps + i] =
            Lfh + Lgh1 * a0 + Lgh2 * w0 + alpha * h;
      }
    }
  }
...
std::vector<double> MPCC::solve(const Eigen::VectorXd& state) {
  ...
  if (!ok) {
    std::cerr << "Error occured during solve (" << solution.status << ")"
              << std::endl;
    // print out state
  ...
  // ------------" << std::endl; std::cout << "max_angvel:" << _max_angvel
  // <<std::endl; std::cout << "max_linvel:" << _max_linvel <<std::endl;
  ...
  _state << solution.x[_x_start], solution.x[_y_start],
      solution.x[_theta_start], solution.x[_v_start], solution.x[_s_start],
