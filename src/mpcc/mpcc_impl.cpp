...
class FG_eval
{
   public:
    ...
    void load_params(const std::map<std::string, double> &params)
    {
        ...
        _ref_etheta =
            params.find("REF_ETHETA") != params.end() ? params.at("REF_ETHETA") : _ref_etheta;
        ...
        _padding =
            params.find("CBF_PADDING") != params.end() ? params.at("CBF_PADDING") : _padding;
    ...
    void operator()(ADvector &fg, const ADvector &vars)
    {
        ...
        for (int i = 0; i < _mpc_steps; i++)
        {
            ...
            AD<double> e_l =
                (-CppAD::cos(ref_phi) * (x - ref_x) - CppAD::sin(ref_phi) * (y - ref_y));
        ...
        for (int i = 0; i < _mpc_steps - 1; i++)
        {
            ...
            if (_use_cbf)
            {
                ...
                AD<double> dist =
                    _dist_grid_ptr->atPositionSafe(CppAD::Value(CppAD::Var2Par(x0)),
...
void MPCC::load_params(const std::map<std::string, double> &params)
{
    ...
    if (_use_cbf) std::cout << "[MPCC] Using CBF!" << std::endl;

    std::cout << "\n!! MPC Obj parameters updated !! " << std::endl;
}

void MPCC::set_tubes(const std::vector<SplineWrapper> &tubes) { _tubes = tubes; }
...
std::vector<double> MPCC::solve(const Eigen::VectorXd &state)
{
    ...
    if (_use_cbf) n_constraints += 2 * _mpc_steps;
    ...
    if (_use_cbf) fg_eval.set_tubes(_tubes);
    ...
    _state << solution.x[_x_start], solution.x[_y_start], solution.x[_theta_start],
        solution.x[_v_start], solution.x[_s_start], solution.x[_s_dot_start];
